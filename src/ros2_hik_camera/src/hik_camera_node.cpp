...
void HikCameraNode ::processImage(cv::Mat &rgb_image) {
    ...
    if (kf_initialized) {
      ...
      if (lost_count_ > Params::loss_thres) {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Target lost for too long, Kalman reset");
      }
    }
  }
  ...
  if (filtered_center.x != -1) {
    ...
    cv::putText(viz, "dyaw: " + std::to_string(filtered_center.x),
...
void HikCameraNode ::init() {
  ...
  // pub
  dyaw_pub_ =
      this->template create_publisher<std_msgs::msg::Float64>("dyaw", 10);
...
void HikCameraNode::declareParams() {
  ...
  for (auto &param : Params::string_params) {
    this->declare_parameter(param.name, param.default_value);
    param.storage_ref =
        this->get_parameter(param.name).get_value<std::string>();
  }
...
void HikCameraNode::paramsEventCB(
    const rcl_interfaces::msg::ParameterEvent &event) {
  for (const auto &changed_param : event.changed_parameters) {
    auto int_it =
        std::find_if(Params::int_params.begin(), Params::int_params.end(),
    ...
    auto double_it =
        std::find_if(Params::double_params.begin(), Params::double_params.end(),
                     [&changed_param](const auto &param) {
                       return param.name == changed_param.name;
                     });
    ...
    auto bool_it =
        std::find_if(Params::bool_params.begin(), Params::bool_params.end(),
                     [&changed_param](const auto &param) {
                       return param.name == changed_param.name;
                     });
    ...
    auto string_it =
        std::find_if(Params::string_params.begin(), Params::string_params.end(),
                     [&changed_param](const auto &param) {
                       return param.name == changed_param.name;
                     });
...
cv::Point2f HikCameraNode::detectYOLO(cv::Mat &origin, cv::Mat &viz,
                                      float size = 640.0) {
  ...
  yolo_openvino.yolov5_detector(model, origin, viz, output_box,
                                output_confidence, size, Params::conf_thres,
                                Params::nms_thres);
  ...
  for (size_t i = 0; i < output_box.size(); ++i) {
    ...
    if (box.br().x < 0 || box.br().y < 0 || box.tl().x < 0 || box.tl().y < 0 ||
        box.br().x > origin.cols || box.br().y > origin.rows ||
        box.tl().x > origin.cols || box.tl().y > origin.rows)
      return target;
    ...
    cv::findContours(binary, contours_, cv::RETR_EXTERNAL,
    ...
    if (!contours_.empty()) {
      ...
      det.box = box;
      det.center = cv::Point((rect.tl().x + rect.br().x) / 2,
                             (rect.tl().y + rect.br().y) / 2);
  ...
  if (candidates.size() > 0) {
    ...
    roi_pub_.publish(
        cv_bridge::CvImage(header, "bgr8", origin(best.box)).toImageMsg());
  }
...
void HikCameraNode::kalmanInit() {
  ...
  kf.processNoiseCov =
      (cv::Mat_<float>(4, 4) << processNoisePos, 0, 0, 0, 0, processNoisePos, 0,
       0, 0, 0, processNoiseVel, 0, 0, 0, 0, processNoiseVel);
...
HikCameraNode::HikCameraNode(const rclcpp::NodeOptions &options)
  ...
  image_buffer_.resize(img_info_.nHeightMax * img_info_.nWidthMax *
                       3); // RGB buffer
  // Init convert param
  ...
  capture_thread_ = std::thread{[this]() -> void {
    ...
    while (rclcpp::ok()) {
      ...
      if (MV_OK == nRet) {
        ...
        // 创建OpenCV Mat
        cv::Mat rgb_image(out_frame.stFrameInfo.nHeight,
